...
CREATE POLICY "Users can view own export requests"
  ON data_export_requests FOR SELECT USING (user_id = (SELECT auth.uid()));
...
CREATE POLICY "Users can create export requests"
  ON data_export_requests FOR INSERT WITH CHECK (user_id = (SELECT auth.uid()));

-- AUDIT_LOG (1)
DROP POLICY IF EXISTS "Users can view own audit log" ON audit_log;
CREATE POLICY "Users can view own audit log"
  ON audit_log FOR SELECT USING (user_id = (SELECT auth.uid()));

-- CONTENT_REPORTS (2)
DROP POLICY IF EXISTS "Users can view own reports" ON content_reports;
CREATE POLICY "Users can view own reports"
  ON content_reports FOR SELECT USING (reporter_id = (SELECT auth.uid()));

DROP POLICY IF EXISTS "Users can create reports" ON content_reports;
CREATE POLICY "Users can create reports"
  ON content_reports FOR INSERT WITH CHECK (reporter_id = (SELECT auth.uid()));

-- BLOCKED_USERS (3)
DROP POLICY IF EXISTS "Users can view own blocked list" ON blocked_users;
CREATE POLICY "Users can view own blocked list"
  ON blocked_users FOR SELECT USING (user_id = (SELECT auth.uid()));

DROP POLICY IF EXISTS "Users can block others" ON blocked_users;
CREATE POLICY "Users can block others"
  ON blocked_users FOR INSERT WITH CHECK (user_id = (SELECT auth.uid()));

DROP POLICY IF EXISTS "Users can unblock" ON blocked_users;
CREATE POLICY "Users can unblock"
  ON blocked_users FOR DELETE USING (user_id = (SELECT auth.uid()));

-- INSTITUTE_RATINGS (3)
DROP POLICY IF EXISTS "Users can create ratings" ON institute_ratings;
CREATE POLICY "Users can create ratings"
  ON institute_ratings FOR INSERT WITH CHECK (user_id = (SELECT auth.uid()));

DROP POLICY IF EXISTS "Users can update own ratings" ON institute_ratings;
CREATE POLICY "Users can update own ratings"
  ON institute_ratings FOR UPDATE USING (user_id = (SELECT auth.uid()));

DROP POLICY IF EXISTS "Users can delete own ratings" ON institute_ratings;
CREATE POLICY "Users can delete own ratings"
  ON institute_ratings FOR DELETE USING (user_id = (SELECT auth.uid()));

-- POST_SHARES (1)
DROP POLICY IF EXISTS "Users can create shares" ON post_shares;
CREATE POLICY "Users can create shares"
  ON post_shares FOR INSERT WITH CHECK (user_id = (SELECT auth.uid()));

-- CONVERSATIONS (2)
DROP POLICY IF EXISTS "Users can view own conversations" ON conversations;
CREATE POLICY "Users can view own conversations"
  ON conversations FOR SELECT USING (user1_id = (SELECT auth.uid()) OR user2_id = (SELECT auth.uid()));

DROP POLICY IF EXISTS "Users can create conversations" ON conversations;
CREATE POLICY "Users can create conversations"
  ON conversations FOR INSERT WITH CHECK (user1_id = (SELECT auth.uid()) OR user2_id = (SELECT auth.uid()));

-- USER_PROFILES - Multiple Permissive Policies (4 warnings)
DROP POLICY IF EXISTS "Public profiles are viewable by everyone" ON user_profiles;
DROP POLICY IF EXISTS "Users can view own profile" ON user_profiles;
CREATE POLICY "Users can view profiles"
  ON user_profiles FOR SELECT USING (true);

SELECT '✅ 48 policy auth RLS ottimizzate!' as status;
